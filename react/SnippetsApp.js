...
const SnippetsApp = ({ api }) => {
  ...
  useEffect(() => {
    api.getSnippets().then(setSnippets).catch((err) => setError(err.message || "Error fetching snippets"));
  }, [api]);
        ...
        <form onSubmit={handleSubmit} className="snippetsapp-form">
          {fields.map((field) => (
            <div key={field.name} className="snippetsapp-field">
              <label htmlFor={`snippetsapp-field-${field.name}`} className="snippetsapp-label">
